 ...
 * 
 * 
 *      MessageSource ：管理 message ，实现国际化等功能。 </br>
 *      ApplicationEventPublisher ：事件发布。 </br>
 *      ResourcePatternResolver ：多资源加载。 </br>
 *      EnvironmentCapable ：系统 Environment（profile + Properties）相关。 </br>
 *      Lifecycle ：管理生命周期。 </br>
 *      Closable ：关闭，释放资源 </br>
 *      InitializingBean：自定义初始化。 </br>
 *      BeanNameAware：设置 beanName 的 Aware 接口。 </br>
 *      会自动初始化非懒加载的 Bean 对象们。
 *      
 *  三种较常见的 ApplicationContext 实现方式：
 *  	1、ClassPathXmlApplicationContext ：从 ClassPath 的 XML 配置文件中读取上下文，并生成上下文定义。应用程序上下文从程序环境变量中取得。</br>
 *  	2、FileSystemXmlApplicationContext ：由文件系统中的XML配置文件读取上下文。</br>
